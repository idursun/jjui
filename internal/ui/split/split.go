...
func (s *SplitState) SetPosition(position config.PreviewPosition) {
	s.AutoPosition = position == config.PreviewPositionAuto
	s.AtBottom = position == config.PreviewPositionBottom
}
...
var _ common.ImmediateModel = (*Split)(nil)
...
func New(state *SplitState, primary, secondary common.ImmediateModel) *Split {
	return &Split{
		State:            state,
		Primary:          primary,
		Secondary:        secondary,
...
func (s *Split) ViewModels(dl *render.DisplayContext, box layout.Box, primary, secondary common.ImmediateModel, vertical bool) {
	if s == nil {
