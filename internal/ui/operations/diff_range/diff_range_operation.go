...
func (o *Operation) Render(commit *jj.Commit, renderPosition operations.RenderPosition) string {
	if renderPosition == operations.RenderPositionBefore && o.from != nil && commit.GetChangeId() == o.from.GetChangeId() {
		...
		return lipgloss.JoinHorizontal(0, sourceMarkerStyle.Render("<< from >>"), dimmedStyle.Render("diff excludes this revision"))
	}

	if renderPosition == operations.RenderBeforeChangeId && o.to != nil && commit.GetChangeId() == o.to.GetChangeId() {
		targetMarkerStyle := common.DefaultPalette.Get("diff_range target_marker")
