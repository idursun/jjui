...
type IViewManagerAccessor interface {
...
func (vm *ViewManager) UpdateLayout(root ILayoutConstraint) {
	vm.layout = root
}
...
func (vm *ViewManager) calculateLayout(layout ILayoutConstraint, availWidth, availHeight int) (int, int) {
	...
	if constraint.ViewId != nil {
		...
		if view != nil && view.Visible {
			...
			if constraint.IsFit {
				// For "Fit" constraint, we could use content size, but for now use a minimal size
...
func (vm *ViewManager) GetViewsNeedsRefresh() []*BaseView {
	var ret []*BaseView
