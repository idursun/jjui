...
func (s reviewSource) AnnotationLocation(start, end int) (lineRange, lineRange, string) {
	...
	if s.presentation == filePresentation {
		...
		if s.usesOldLines() {
			return lines, lineRange{}, diffContextSnippet(s.file.Content[start : end+1])
		}
...
func (s reviewSource) Contains(index int, annotation Annotation) bool {
	...
	if s.presentation == filePresentation {
		return lineInRange(index+1, s.annotationLines(annotation))
	}
...
func (s reviewSource) Anchor(annotation Annotation) (int, bool) {
	if s.presentation == filePresentation {
		lines := s.annotationLines(annotation)
		if lines.End <= 0 || lines.End > s.Len() {
			return 0, false
		}
...
func (s reviewSource) usesOldLines() bool {
	return s.presentation == filePresentation &&
		s.file != nil &&
		s.file.Patch != nil &&
		s.file.Patch.NewPath == ""
}

func (s reviewSource) annotationLines(annotation Annotation) lineRange {
	if s.usesOldLines() {
		return annotation.OldLines
	}
	return annotation.NewLines
}
