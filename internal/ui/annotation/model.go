...
func (m *Model) HandleIntent(intent intents.Intent) (tea.Cmd, bool) {
	...
	case intents.AnnotationAdd:
		if annotation, ok := m.annotationAtCursor(); ok {
			m.focusedAnnotationID = annotation.ID
			m.startEditorFor(annotation)
		} else if m.currentCommentable() {
			m.startEditor()
		}
		return nil, true
	case intents.AnnotationDelete:
		if annotation, ok := m.annotationAtCursor(); ok {
			m.removeAnnotation(annotation.ID)
...
func (m *Model) Update(msg tea.Msg) tea.Cmd {
	...
	case revisionLoadedMsg:
		initialLoad := m.document.loading && msg.ChangeID == m.changeID()
		pendingLoad := msg.ChangeID == m.document.loadingRevision
		if !initialLoad && !pendingLoad {
			return nil
		}
		if pendingLoad {
...
func (m *Model) ViewRect(dl *render.DisplayContext, box layout.Box) {
	...
	m.viewportWidth = result.viewportWidth
	m.viewportHeight = result.viewportHeight
...
func (m *Model) openTargetPicker() tea.Cmd {
	...
	files := make([]string, 0, len(m.document.files))
...
func (m *Model) commentPickerLabel(annotation Annotation) string {
	...
	return fmt.Sprintf("%s · %s:%s · %s", shortRevision(annotation.ChangeID), annotation.File, annotationHeaderRange(annotation), preview)
}
...
func (m *Model) navigateRevision(direction revisionDirection) tea.Cmd {
	...
	return m.loader.LoadRevisionTargets(m.changeID(), direction)
}
...
func (m *Model) loadRevision(changeID string) tea.Cmd {
	return m.loader.LoadRevision(changeID, m.document.presentation == filePresentation)
}
