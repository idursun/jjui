...
func TestStaleRevisionTargetResponseCannotOverrideLatestDirection(t *testing.T) {
	runner := test.NewTestCommandRunner(t)
	defer runner.Verify()
	runner.Expect(jj.GetRevisionSummariesFromRevset("current-")).SetOutput([]byte("parent\tParent\n"))
	runner.Expect(jj.GetRevisionSummariesFromRevset("current+")).SetOutput([]byte("first\tFirst\nsecond\tSecond\n"))

	model := New(test.NewTestContext(runner), "current")
	parentCmd := model.navigateRevision(revisionParent)
	childCmd := model.navigateRevision(revisionChild)
	require.NotNil(t, parentCmd)
	require.NotNil(t, childCmd)

	parentResponse, ok := parentCmd().(revisionTargetsLoadedMsg)
	require.True(t, ok)
	childResponse, ok := childCmd().(revisionTargetsLoadedMsg)
	require.True(t, ok)
	assert.NotEqual(t, parentResponse.RequestID, childResponse.RequestID)

	assert.Nil(t, model.Update(parentResponse))
	assert.Equal(t, childResponse.RequestID, model.revisionTargetsRequestID)

	openPicker := model.Update(childResponse)
	require.NotNil(t, openPicker)
	message, ok := openPicker().(common.OpenTargetPickerMsg)
	require.True(t, ok)
	items, err := message.Sources[0].Fetch(nil)
...
func TestRevisionTargetErrorClearsPendingRequest(t *testing.T) {
	runner := test.NewTestCommandRunner(t)
	defer runner.Verify()
	runner.Expect(jj.GetRevisionSummariesFromRevset("current-"))
	runner.Expect(jj.GetRevisionSummariesFromRevset("current+")).SetError(errors.New("target discovery failed"))
	...
	require.NotNil(t, childCmd)
	_ = parentCmd()
	response, ok := childCmd().(revisionTargetsLoadedMsg)
	require.True(t, ok)

	showError := model.Update(response)
	require.NotNil(t, showError)
	assert.Zero(t, model.revisionTargetsRequestID)
	message, ok := showError().(intents.AddMessage)
	require.True(t, ok)
	assert.Equal(t, "target discovery failed", message.Text)
}

func TestStaleRevisionLoadResponseCannotReplaceLatestRevision(t *testing.T) {
	runner := test.NewTestCommandRunner(t)
	defer runner.Verify()
	for _, revision := range []string{"first", "second"} {
		runner.Expect(jj.GetDescription(revision)).SetOutput([]byte(revision + " description"))
		runner.Expect(jj.AnnotationDiff(revision)).SetOutput([]byte("diff --git a/" + revision + ".go b/" + revision + ".go\n"))
	}

	model := New(test.NewTestContext(runner), "current")
	model.document.loading = false
	firstLoad := model.selectRevision("first")
	secondLoad := model.selectRevision("second")
	require.NotNil(t, firstLoad)
	require.NotNil(t, secondLoad)

	firstResponse, ok := firstLoad().(revisionLoadedMsg)
	require.True(t, ok)
	secondResponse, ok := secondLoad().(revisionLoadedMsg)
	require.True(t, ok)
	assert.NotEqual(t, firstResponse.RequestID, secondResponse.RequestID)

	assert.Nil(t, model.Update(firstResponse))
	assert.Equal(t, "current", model.changeID())
	assert.Equal(t, "second", model.document.loadingRevision)

	assert.Nil(t, model.Update(secondResponse))
	assert.Equal(t, "second", model.changeID())
	assert.Empty(t, model.document.loadingRevision)
	assert.Zero(t, model.revisionLoadRequestID)
}

func TestRevisionNavigationDoesNotDiscardEditorDraft(t *testing.T) {
	runner := test.NewTestCommandRunner(t)
	defer runner.Verify()
	runner.Expect(jj.GetDescription("target")).SetOutput([]byte("Target"))
	runner.Expect(jj.AnnotationDiff("target")).SetOutput([]byte("diff --git a/target.go b/target.go\n"))

	model := annotationTestModel()
	model.context = test.NewTestContext(runner)
	model.loader.context = model.context
	model.startEditor()
	model.editor.SetValue("draft comment")

	blocked := model.selectRevision("target")
	require.NotNil(t, blocked)
	message, ok := blocked().(intents.AddMessage)
	require.True(t, ok)
	assert.Contains(t, message.Text, "Finish editing")
	assert.Equal(t, "commit", model.changeID())
	assert.Empty(t, model.document.loadingRevision)
	assert.True(t, model.editing)
	assert.Equal(t, "draft comment", model.editor.Value())

	model.cancelEditor()
	load := model.selectRevision("target")
	require.NotNil(t, load)
	model.startEditor()
	model.editor.SetValue("draft while loading")
	messageCmd := model.Update(load())
	require.NotNil(t, messageCmd)
	message, ok = messageCmd().(intents.AddMessage)
	require.True(t, ok)
	assert.Contains(t, message.Text, "Finish editing")
	assert.Equal(t, "commit", model.changeID())
	assert.Empty(t, model.document.loadingRevision)
	assert.True(t, model.editing)
	assert.Equal(t, "draft while loading", model.editor.Value())
}
...
func TestTargetPickerListsChangedFilesAndSelectsOne(t *testing.T) {
	...
	model.Update(target_picker.TargetSelectedMsg{File: jj.NewFileName("second.go"), Payload: targetPickerPayload{}})
...
func TestCommentPickerSelectionFallsBackToFullFileOutsideDiff(t *testing.T) {
	runner := test.NewTestCommandRunner(t)
	defer runner.Verify()
	runner.Expect(jj.GetDescription("change")).SetOutput([]byte("Current revision"))
	runner.Expect(jj.AnnotationDiff("change")).SetOutput([]byte(`diff --git a/a.go b/a.go
--- a/a.go
+++ b/a.go
@@ -1 +1 @@
 one`))
	runner.Expect(jj.FileShow("change", "a.go")).SetOutput([]byte("one\noutside the hunk"))
	...
	require.NotNil(t, load)
	assert.Equal(t, filePresentation, model.document.presentation)
	require.Nil(t, model.Update(load()))
...
func TestCommentPickerSelectionLoadsFullFileAfterRevisionChange(t *testing.T) {
	runner := test.NewTestCommandRunner(t)
	defer runner.Verify()
	runner.Expect(jj.GetDescription("target")).SetOutput([]byte("Target revision"))
	runner.Expect(jj.AnnotationDiff("target")).SetOutput([]byte(`diff --git a/a.go b/a.go
--- a/a.go
+++ b/a.go
@@ -1 +1 @@
 one`))
	runner.Expect(jj.FileShow("target", "a.go")).SetOutput([]byte("one\noutside the hunk"))
	...
	annotation := model.addAnnotation(Annotation{
		...
		File:     "a.go",
		NewLines: lineRange{Start: 2, End: 2},
		Comment:  "review target line",
	})

	loadRevision := model.Update(target_picker.TargetSelectedMsg{
		Target:  strconv.Itoa(annotation.ID),
		Payload: commentPickerPayload{},
	})
	require.NotNil(t, loadRevision)
	loadFile := model.Update(loadRevision())
	require.NotNil(t, loadFile)
	assert.Equal(t, "target", model.changeID())
	assert.Equal(t, filePresentation, model.document.presentation)
	require.Nil(t, model.Update(loadFile()))
...
func TestCommentPickerSelectionReportsInvalidFullFileRange(t *testing.T) {
	file := fileItem{
		Path:          "a.go",
		Content:       []string{"one", "two"},
		ContentLoaded: true,
		Patch:         &patchFile{NewPath: "a.go"},
	}
	model := New(nil, "change")
	model.document.loading = false
	model.document.files = []fileItem{file}
	annotation := model.addAnnotation(Annotation{
		ChangeID: "change",
		File:     "a.go",
	...
	message := model.Update(target_picker.TargetSelectedMsg{
		Target:  strconv.Itoa(annotation.ID),
		Payload: commentPickerPayload{},
	})
	require.NotNil(t, message)
	assert.Zero(t, model.focusedAnnotationID)
	result, ok := message().(intents.AddMessage)
	require.True(t, ok)
	assert.Equal(t, "Annotation location is unavailable", result.Text)
}
...
func TestAnnotationAtCursorDistinguishesMatchCount(t *testing.T) {
	model := annotationTestModel()
	model.cursor = 1

	assert.Empty(t, model.annotationAtCursor())

	model.addAnnotation(Annotation{ChangeID: "commit", File: "a.go", NewLines: lineRange{Start: 1, End: 2}, Comment: "first"})
	assert.Len(t, model.annotationAtCursor(), 1)

	model.addAnnotation(Annotation{ChangeID: "commit", File: "a.go", NewLines: lineRange{Start: 2, End: 2}, Comment: "second"})
...
func TestAmbiguousAnnotationActionsOpenCommentPicker(t *testing.T) {
	tests := []struct {
		...
		{name: "add", intent: intents.AnnotationAdd{}},
		{name: "delete", intent: intents.AnnotationDelete{}},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			model.addAnnotation(Annotation{ChangeID: "commit", File: "a.go", NewLines: lineRange{Start: 1, End: 2}, Comment: "first"})
			model.addAnnotation(Annotation{ChangeID: "commit", File: "a.go", NewLines: lineRange{Start: 2, End: 2}, Comment: "second"})
			model.addAnnotation(Annotation{ChangeID: "commit", File: "other.go", NewLines: lineRange{Start: 2, End: 2}, Comment: "unrelated"})

			cmd, handled := model.HandleIntent(tt.intent)

			require.True(t, handled)
			require.NotNil(t, cmd)
			message, ok := cmd().(common.OpenTargetPickerMsg)
			require.True(t, ok)
			assert.IsType(t, commentPickerPayload{}, message.Payload)
			require.Len(t, message.Sources, 1)
			items, err := message.Sources[0].Fetch(nil)
			require.NoError(t, err)
			assert.Len(t, items, 2)
			assert.Equal(t, []string{"1", "2"}, []string{items[0].Value, items[1].Value})
			assert.Len(t, model.annotations.All(), 3)
...
func TestAnnotationHalfPageNavigation(t *testing.T) {
	lines := make([]patchLine, 10)
	for i := range lines {
		lines[i] = patchLine{
			Kind:    lineContext,
	...
	_, handled := model.HandleIntent(intents.AnnotationMove{Delta: 1, HalfPage: true})
	assert.True(t, handled)
...
func TestWrappedLinesAndAnnotationsPositionCursorAndScroll(t *testing.T) {
	model := mouseTestModel(2)
	model.renderer = newAnnotationRenderer(false)
	model.document.files[0].Patch.Lines[0].Content = "abcdefghijk"
	model.document.files[0].Patch.Lines[1].Content = "second"
	model.wrap = true
	model.viewportWidth = 17
	model.viewportHeight = 2
	model.cursor = 1
	model.addAnnotation(Annotation{
		ChangeID: "commit",
		File:     "a.go",
		NewLines: lineRange{Start: 1, End: 1},
		Comment:  "x",
	})

	model.ensureCursorVisible()

	assert.Equal(t, 4, model.scrollY)
	display := render.NewDisplayContext()
	model.ViewRect(display, layout.NewBox(layout.Rect(0, 0, 17, 3)))
	...
	require.True(t, handled)
	assert.Equal(t, lineClickedMsg{SourceIndex: 1}, message)
}
...
func TestViewRectResizesEditorBeforeRendering(t *testing.T) {
	model := annotationTestModel()
	model.startEditor()
	model.editor.SetValue("a comment long enough to wrap")
	model.editor.MoveToEnd()

	wideDisplay := render.NewDisplayContext()
	model.ViewRect(wideDisplay, layout.NewBox(layout.Rect(0, 0, 50, 8)))
	wideWidth := model.editor.Width()
	wideCursor := wideDisplay.Cursor()
	require.NotNil(t, wideCursor)

	narrowDisplay := render.NewDisplayContext()
	model.ViewRect(narrowDisplay, layout.NewBox(layout.Rect(0, 0, 30, 8)))
	narrowWidth := model.editor.Width()
	narrowCursor := narrowDisplay.Cursor()
	require.NotNil(t, narrowCursor)

	assert.Less(t, narrowWidth, wideWidth)
	assert.Greater(t, narrowCursor.Position.Y, wideCursor.Position.Y)
}
