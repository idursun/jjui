...
func (m *Model) RenderConfirmationOverlay(dl *render.DisplayContext, box layout.Box) {
...
func (m *Model) renderTopLevelRow(tb *render.TextBuilder, row visibleRow, group bookmarkTreeItem, node bookmarkRowNode) {
	...
	// Remote rows in mixed groups show only their own remote name.
	if node.IsRemote() && !group.remoteOnly() {
