...
func dialServer(addr, key string) error {
	...
	// check if the prompt was succesful
	one := make([]byte, 1)
	n, err := conn.Read(one)
	if n == 0 || err != nil {
...
func (s *Server) smokeTestPeerCred(ln net.Listener) error {
	go func() {
...
func (s *Server) handle(conn net.Conn, askpass func(name, prompt string, done <-chan struct{}) (string, bool)) error {
	defer conn.Close()
