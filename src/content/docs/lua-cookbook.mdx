...
### Open the next revision's details

When the details view is open, pressing `shift+j` closes it, moves down one revision, and opens the details view for the newly highlighted revision.

```lua
-- ~/.config/jjui/config.lua
function setup(config)
  config.action("shift details down", function()
    revisions.details.close()
    revisions.navigate({ by = 1 })
...
`revisions.details.close()` closes the details view before `revisions.navigate()` changes the selection in the underlying revision list.
...
Pressing `N` creates a new revision, opens the inline describe editor, and repeats — letting you rapidly commit multiple changes in succession. Cancel the describe prompt (e.g. press `Escape`) to break out of the loop.
...
`wait_close()` blocks until the inline describe editor is dismissed and returns `false` when the user cancels, which breaks the loop. `wait_refresh()` ensures the revision list is up to date before proceeding.
