...
Preview commands are run with `COLUMNS` and `LINES` set to the current preview pane size. `jjui` also sets `DFT_WIDTH` for difftastic-based previews so they wrap to the preview pane width automatically.
