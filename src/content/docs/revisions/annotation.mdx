...
The view starts on the revision's diff and displays one changed file at a time. Comments are kept in memory while the view is open; they are not written back to the repository. Press `y` to copy all comments as Markdown to the system clipboard.

Press `?` to expand the current view's help. When help is expanded, `esc`
closes the help first; otherwise it closes the Annotation View.
