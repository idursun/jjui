...
![GIF](/jjui/gifs/jjui_details_restore.gif)
...
- `d`: diff highlighted file
- `m` / `space`: toggle-select file
- `r`: restore selected files
- `s`: split selected files
- `alt+s`: split parallel
- `shift+s`: squash selected files
- `shift+a`: absorb selected files
- `*`: filter revset to revisions touching this file
- `p`: toggle preview
- `ctrl+r`: refresh
- `esc` / `left` / `h`: close
