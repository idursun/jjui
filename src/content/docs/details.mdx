...
File paths are displayed relative to the directory where jjui was started, which keeps paths short when working in a subdirectory. jjui still uses repository-relative paths when running commands.
...
Press `/` to filter changed files by a case-insensitive path substring. Matching text is highlighted, and existing file selections are preserved while filtering.
...
- `i`: invert the current file selection
- Most operations act on selected files. If no files are selected, the operation uses the highlighted file.

Inversion applies to the complete file list, including files hidden by an
active filter. If nothing is selected, jjui first treats the highlighted file
as selected, so the other files become selected.
