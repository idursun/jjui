...
When choosing an individual file, file paths are displayed relative to the directory where jjui was started.
...
If you accept without choosing a different target, jjui omits the redundant
`--to` argument and runs `jj diff --from <from>` against the working copy.
