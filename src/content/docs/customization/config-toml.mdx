---
title: config.toml
description: Configure jjui with config.toml
---

jjui reads configuration from a TOML file at startup.

**Global config:**
- Linux / macOS: `~/.config/jjui/config.toml`
- Windows: `%AppData%/jjui/config.toml`

Open the file directly in your editor with:

```bash
jjui --config
```

**Repo-specific config:**
You can create a `.jjui/config.toml` file in the root of any jj repo and it will *override* anything that was set by your global config. This allows you to define settings, actions, and bindings that apply to all repos, but then granularly override particular settings per-repo.

**Custom config:**
You can also set a custom directory with the `JJUI_CONFIG_DIR` environment variable. jjui will then look for your `config.toml` file inside that directory. If you use this, it will *replace* the global and repo configs, rather than just override them.

---

## Customizing jj inside jjui

`jjui` sets the `JJUI` environment variable before invoking `jj`. You can use `jj`'s [conditional config syntax](https://docs.jj-vcs.dev/latest/config/#conditional-variables) to apply different `jj` settings only when commands are run from inside `jjui`.

This is useful when you want `jj` to behave one way in your shell and another way in `jjui`.

### Enable a diff formatter only in jjui

```toml
[[--scope]]
--when.environments = ["JJUI"]
[--scope.ui]
diff-formatter = "delta"
```

### Use a different pager in jjui

```toml
[[--scope]]
--when.environments = ["JJUI"]
[--scope.ui]
pager = ["less", "-FRX"]
```

### Disable pagination in jjui

```toml
[[--scope]]
--when.environments = ["JJUI"]
[--scope.ui]
paginate = "never"
```

Add these snippets to your `jj` configuration, not to `jjui`'s `config.toml`.

## Revisions

Override the default log template or revset:

```toml
[revisions]
template = "builtin_log_compact"
revset = ""
log_batching = true
log_batch_size = 50
```

- `log_batching`: load revisions incrementally in batches (default: `true`)
- `log_batch_size`: number of revisions per batch when batching is enabled (default: `50`)

## Preview

```toml
[preview]
position = "auto"          # auto | right | bottom
show_at_start = false
width_percentage = 50.0
width_increment_percentage = 5.0
```

## Op Log

```toml
[oplog]
limit = 200
```

## Interactive bookmark pane

Enable the experimental split-pane bookmark browser. The default is `false`, which keeps the classic bookmark menu:

```toml
[bookmark]
interactive_bookmark_pane = true
```

When enabled, `b` toggles the pane and `tab` switches focus between it and the revisions view. See [Bookmarks](/jjui/bookmarks#interactive-bookmark-pane) for its actions.

## Credential prompts

jjui handles Git and SSH credential requests in an in-app askpass prompt by
default. Usernames are visible, while passwords and passphrases are masked.
Disable this if you manage credentials with your own helpers or prompt:

```toml
[askpass]
enabled = false
```

The older `[ssh].hijack_askpass` setting has been removed.

## Exec suggestions

Controls autocomplete suggestions in the `:` and `$` command prompts:

```toml
[suggest.exec]
mode = "fuzzy"   # off | regex | fuzzy
```

## Auto-refresh

```toml
[ui]
auto_refresh_interval = 0
```

Seconds between automatic revision list refreshes. Default is `0` (disabled). Override at startup with `--period`.

## Window title

```toml
[ui]
set_window_title = true
```

Controls the terminal window title. Default is `true`.

- `true` — show the full repo path (e.g. `jjui - /home/user/projects/my-repo`)
- `"full"` — same as `true`
- `"base"` — show only the directory name (e.g. `jjui - my-repo`)
- `false` — leave the title managed by your shell or terminal

## Mouse support

```toml
[ui]
mouse_support = true
```

Whether jjui enables mouse interactions such as clicking list items, scrolling scrollable views, and dragging pane separators. Default is `true`. Set to `false` to disable mouse handling completely.

## Flash messages

```toml
[ui]
flash_message_display_seconds = 4
```

How long success flash messages stay visible in seconds. Default is `4`. Set to `0` to keep them until manually dismissed with `esc`.

## Colors and themes

Quick inline overrides go in `[ui.colors]`:

```toml
[ui.colors]
":selected" = { bg = "#303446" }
```

Control how strongly selected backgrounds blend toward the surrounding surface with `ui.background_blend`. Values range from `0.0` (disabled) to `1.0`:

```toml
[ui]
background_blend = 0.4
```

You can also set separate values for light and dark terminal appearances:

```toml
[ui]
background_blend = { light = 0.2, dark = 0.4 }
```

The config value overrides the blend value supplied by the active theme.

For a full theme file, see [Themes](/jjui/customization/themes).

## Bindings profile

Load an alternative bindings file as the base instead of the built-in defaults:

```toml
bindings_profile = "vim_bindings.toml"
```

Your own `[[bindings]]` entries are applied on top of the profile. Use `:builtin` to explicitly reset to embedded defaults.

For defining actions and bindings, see [Actions and Bindings](/jjui/customization/actions-and-bindings).
