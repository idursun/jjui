...
This installs generated jjui Lua API metadata into your config directory. See [config.lua](/jjui/customization/config-lua#editor-support) for details.
...
Other built-in action scopes (`ui`, `git`, `diff`, `oplog`, `bookmarks`, `bookmark_pane`, `annotation`, `undo`, `redo`, `choose`, `input`, `command_history`, `file_search`) are **only** available under the `jjui` namespace — e.g. `jjui.git.push()`, not `git.push()`.
...
The selection getters read live UI state when called. After a yielding action or
refresh, they reflect the current selection rather than a cached snapshot.
...
| `jj_async(...args)` | — | Queues a command without waiting; its output or error is shown as a flash message when it completes |
...
`jj_async` does not return command output to Lua or wait for the command before Lua continues. Use `jj` when later Lua code needs the command's output or must run only after it has completed.
...
| `choose({ options, title?, ordered? })` | string or nil | Table form; press `/` in any choice dialog to open the `filter:` prompt |
| `input({ title?, prompt?, value? })` | string or nil | Show a text input dialog; `value` sets its initial text; returns the entered text or nil |
...
| `change_workspace(path)` | `(true, nil)` or `(nil, err)` | Switch the workspace path used by subsequent `jj` commands |
...
`change_workspace(path)` is also available as `jjui.change_workspace(path)`. It changes the workspace directory for command execution without reloading repo-specific jjui configuration.
...
| `revisions.open_diff_range()` | Enter diff range mode |
| `revisions.open_annotation()` | Open the experimental Annotation View |
...
| Function |
|----------|
| `revisions.absorb.toggle_select()` |
| `revisions.absorb.select_descendants()` |
| `revisions.absorb.apply()` |
| `revisions.absorb.cancel()` |
| `revisions.absorb.ace_jump()` |
| `revisions.absorb.jump_to_working_copy()` |

### revisions.diff_range.*

| Function | Args |
|----------|------|
| `revisions.diff_range.apply()` | |
| `revisions.diff_range.swap()` | |
| `revisions.diff_range.target_picker({ target })` | `"from"` or `"to"` |
| `revisions.diff_range.cancel()` | |
...
| `revisions.inline_describe.clear()` | Clear the editor and save the previous text for `ctrl+y` |
| `revisions.inline_describe.content()` | Return the editor's current text, or `nil` when unavailable |
...
| `revisions.details.invert_selection()` | Invert file selection |
...
| `revisions.details.cancel()` / `close()` | `close()` is an alias for `cancel()` |
...
Shared target picker used by rebase, squash, duplicate, revert, and diff range.
...
| `jjui.ui.toggle_bookmark_pane()` / `jjui.ui.focus_next_pane()` | |
...
These actions are available when the experimental interactive bookmark pane is enabled.

| Function |
|----------|
| `jjui.bookmark_pane.move_up()` / `jjui.bookmark_pane.move_down()` |
| `jjui.bookmark_pane.page_up()` / `jjui.bookmark_pane.page_down()` |
| `jjui.bookmark_pane.cycle_remotes()` / `jjui.bookmark_pane.cycle_remotes_back()` |
| `jjui.bookmark_pane.open_filter()` / `jjui.bookmark_pane.toggle_expand()` |
| `jjui.bookmark_pane.toggle_select()` |
| `jjui.bookmark_pane.show_in_revision()` / `jjui.bookmark_pane.set_revset()` |
| `jjui.bookmark_pane.edit()` / `jjui.bookmark_pane.new()` / `jjui.bookmark_pane.rename()` |
| `jjui.bookmark_pane.create()` / `jjui.bookmark_pane.move()` |
| `jjui.bookmark_pane.delete()` / `jjui.bookmark_pane.forget()` / `jjui.bookmark_pane.track()` / `jjui.bookmark_pane.untrack()` |
| `jjui.bookmark_pane.push()` / `jjui.bookmark_pane.fetch()` |
| `jjui.bookmark_pane.apply()` / `jjui.bookmark_pane.cancel()` |

### jjui.annotation.*

These actions are available in the experimental Annotation View.

| Function |
|----------|
| `jjui.annotation.move_up()` / `move_down()` |
| `jjui.annotation.select_up()` / `select_down()` |
| `jjui.annotation.page_up()` / `page_down()` |
| `jjui.annotation.half_page_up()` / `half_page_down()` |
| `jjui.annotation.move_top()` / `move_bottom()` |
| `jjui.annotation.left()` / `right()` |
| `jjui.annotation.prev_file()` / `next_file()` |
| `jjui.annotation.target_picker()` / `comment_picker()` |
| `jjui.annotation.parent_revision()` / `child_revision()` |
| `jjui.annotation.toggle_presentation()` / `toggle_wrap()` |
| `jjui.annotation.add()` / `delete()` / `clear()` / `copy()` |
| `jjui.annotation.editor.save()` / `jjui.annotation.editor.cancel()` |
